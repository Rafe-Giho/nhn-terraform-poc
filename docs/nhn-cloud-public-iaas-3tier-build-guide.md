# NHN Cloud 공공기관 IaaS 3-tier 구축 가이드

기준 provider: `nhn-cloud/nhncloud` `= 1.0.8`  
대상: 공공기관 클라우드 전환 사업의 VM 기반 Web/WAS/DB 3-tier 구조
...
현재 저장소에는 이 구조의 Terraform 코드가 아직 구현되어 있지 않다. provider 지원 범위상 구현 가능하며, 실제 적용 전 `compute`, `load-balancer`, `block-storage` 모듈과 `infra/envs/public-iaas-dev` stack을 추가해야 한다.
...
환경별 운영에서는 `public-iaas-dev`, `public-iaas-stage`, `public-iaas-prod`처럼 state를 분리한다.
...
| VPN/전용회선/Transit/NAT | 조건부 | 기관망 연동 |
...
아래 명령은 `infra/envs/public-iaas-dev` stack을 추가한 뒤 사용한다.

```bash
cp ./infra/envs/public-iaas-dev/terraform.tfvars.example ./infra/envs/public-iaas-dev/terraform.tfvars

terraform -chdir=infra/envs/public-iaas-dev init -backend=false
terraform -chdir=infra/envs/public-iaas-dev fmt -recursive
terraform -chdir=infra/envs/public-iaas-dev validate
terraform -chdir=infra/envs/public-iaas-dev plan -out=tfplan
terraform -chdir=infra/envs/public-iaas-dev show -json tfplan > ../../../harness/out/public-iaas-dev-plan.json
```

적용은 plan 검토와 승인 후 실행한다.

```bash
terraform -chdir=infra/envs/public-iaas-dev apply tfplan
```
