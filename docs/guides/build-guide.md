...
- [NHN Cloud 네트워크 표준 설계](../standards/architecture/network.md)
- [NHN Cloud 보안 표준 설계](../standards/architecture/security.md)
...
| 배포 방식 | 수동 배포, SCP, Ansible, VM image, CI 연동 | GitLab/Gitea, Jenkins, GitOps, Argo CD, CI runner, image registry |
| 데이터 계층 | VM 기반 DB 또는 별도 Managed DB 검증 | Managed DB 또는 외부 DB 권장, stateful workload는 별도 검증 |
| Terraform 적합도 | 네트워크/보안/VM/LB/볼륨 표준화에 적합 | NKS foundation, DevOps VM, platform add-on 표준화에 적합 |
...
| Internet Gateway ID | 조건부 | public routing table gateway attach |
...
| GitLab/Gitea/Jenkins image/flavor | 조건부 | 클라우드 네이티브 DevOps 연동 서버 |
| Remote state 저장소 | 조건부 | 협업과 운영 state 관리 |
| 보안 요구사항 | 예 | 망 분리, 접근 통제, 로그 보존, 백업 정책 확정 |
| NACL/Flow Log/NAT/Service Gateway | 조건부 | provider 표준 범위 밖 콘솔 선행 구성 |
...
네트워크 표준은 `VPC 1개 + public/private/management routing table + 역할별 subnet`을 기본으로 한다. Public VPC와 Private VPC를 따로 만드는 방식은 기본 표준이 아니라 보안 도메인 또는 프로젝트 경계가 분리될 때만 별도 설계로 적용한다.
...
- NACL, Flow Log, NAT Gateway, Service Gateway처럼 Terraform plan에 나타나지 않는 콘솔 선행 항목 검증 여부
