...
이 가이드는 기존 업무시스템을 NHN Cloud Compute 기반으로 이전할 때 사용한다. Web, WAS, DB 계층을 분리하고, 모니터링/로그/백업/보안 솔루션 서버를 운영망에 배치하는 구조를 기준으로 한다. 네트워크는 Public VPC/Private VPC 2개가 아니라 하나의 VPC 안에서 public/private/management routing table과 역할별 subnet으로 분리한다.
...
| Network ACL | 조건부 | subnet/network 앞단 allow/deny 통제 |
| Flow Log 저장 Object Storage | 조건부 | SG/NACL 검증과 보안 감사 |
| Service Gateway | 조건부 | Object Storage/NCR private 접근 |
| Quota | 예 | instance, volume, LB, Floating IP 생성 가능성 |

VPN, 전용회선, Transit Hub, NAT Gateway, Service Gateway, Network ACL, Flow Log, WAF 같은 항목은 provider 표준 A 범위 밖으로 보고 콘솔 선행 생성 또는 별도 PoC 후 Terraform 편입을 판단한다.

## 4. Terraform 생성/관리 범위

| 영역 | 대표 resource | 기준 |
|---|---|---|
| VPC/Subnet/Routing | `nhncloud_networking_vpc_v2`, `nhncloud_networking_vpcsubnet_v2`, `nhncloud_networking_routingtable_v2`, `nhncloud_networking_routingtable_attach_gateway_v2` | 표준 모듈 |
...
| 계층 | 기본 구성 | 접근 기준 |
|---|---|---|
| DMZ | Public LB, WAF/Reverse Proxy, 승인된 bastion | `public` routing table. 외부는 80/443 또는 승인 포트만 허용 |
| Web | Web 서버 | `private` routing table. DMZ LB/Proxy에서만 Web 포트 허용 |
| Application | Internal LB, WAS 서버, 배치 서버 | `private` routing table. Web 계층 또는 internal LB에서만 WAS 포트 허용 |
| Data | DB 서버, DB volume, 백업 volume, NAS | `private` routing table. WAS 계층에서 DB 포트만 허용 |
| Management | Bastion, 배포 서버, 패치 저장소 | `management` routing table. 관리 CIDR, VPN, 전용회선에서만 접근 |
...
`security` 모듈은 provider 코드의 `delete_default_rules`를 사용해 Security Group 생성 시 자동 추가되는 전체 outbound rule을 삭제한다. 이후 아래 허용 rule만 명시한다.

| SG | Ingress | Egress |
|---|---|---|
| `public_lb` | public CIDR -> TCP 80/443 | web subnet -> TCP `web_port` |
| `internal_lb` | web subnet -> TCP `was_port` | app subnet -> TCP `was_port` |
| `web` | dmz subnet -> TCP `web_port`, 관리 CIDR -> SSH | app subnet -> TCP `was_port`, operations subnet -> UDP `log_ingress_port` |
| `was` | app/internal LB subnet -> TCP `was_port`, 관리 CIDR -> SSH | data subnet -> TCP `db_port`, operations subnet -> UDP `log_ingress_port` |
| `db` | app subnet -> TCP `db_port`, 관리 CIDR -> SSH | operations subnet -> UDP `log_ingress_port` |
| `management` | 관리 CIDR -> SSH | VPC CIDR -> `management_egress_ports` |
| `operations` | VPC CIDR -> UDP `log_ingress_port`, 관리 CIDR -> SSH | VPC CIDR -> `operations_polling_ports` |

사업별 예외는 `extra_security_group_rules`에 추가한다. 예외 rule은 출발지, 목적지, 포트, 사유, 만료일을 설계서에 남긴다.

### NACL/Flow Log 기준

Network ACL은 Terraform provider 표준 A 범위 밖이므로 콘솔/API로 구성한다. 적용 대상은 DMZ, Data, Management/Operations subnet을 우선한다.

Flow Log는 public entry, bastion, DB, operations 서버 NIC를 우선 활성화한다. Flow Log 저장소는 Object Storage를 사용하고, 최초 apply 후 SG/NACL 차단 로그를 확인한다.

## 6. 구축 순서

1. 주소 체계, routing table, subnet, 보안 그룹 기준을 확정한다.
2. `network` 모듈로 VPC, public/private/management routing table, 역할별 subnet을 만든다.
3. public routing table에만 Internet Gateway를 연결한다.
4. `security` 모듈로 DMZ, Web, Application, Data, Management, Operations 보안 그룹을 만든다.
5. `load-balancer` 모듈로 public LB와 internal LB를 구성한다.
6. `compute` 모듈로 Web/WAS/DB/운영 솔루션 서버를 만든다.
7. `block-storage` 모듈로 DB/log/backup volume을 생성하고 instance에 attach한다.
8. `object-storage`와 NAS를 백업, 로그, 배포 산출물 저장소로 구성한다.
9. OS hardening, agent, monitoring, backup client 설치는 image, Ansible, 솔루션 설치 절차로 수행한다.
10. plan JSON으로 삭제/교체, 공개 인바운드, 계층 간 접근 위반을 검토한다.
11. 사용자 승인 후 dev 최소 구성부터 apply한다.
...
| Routing table | network diff/output | Internet Gateway는 `public` routing table에만 연결 |
| Subnet mapping | `subnet_routing_table_keys` output | `dmz=public`, `web/app/data=private`, `management/operations=management` |
...
| 전체 egress | SG rule diff, policy check | 운영 `0.0.0.0/0` 전체 outbound 금지 |
...
| 검증 | 기준 |
|---|---|
| Routing | DMZ만 public routing table, Web/Application/Data는 private, Management/Operations는 management routing table |
...
| Flow Log | public entry, bastion, DB, operations NIC 수집 확인 |
| NACL | 콘솔 rule 순서와 SG rule 충돌 여부 확인 |
