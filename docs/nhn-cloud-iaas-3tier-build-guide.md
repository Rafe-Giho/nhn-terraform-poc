...
현재 저장소에는 이 구조의 Terraform 코드가 아직 구현되어 있지 않다. provider 지원 범위상 구현 가능하며, 실제 적용 전 `compute`, `load-balancer`, `block-storage` 모듈과 `infra/envs/iaas-3tier-dev` stack을 추가해야 한다.
...
환경별 운영에서는 `iaas-3tier-dev`, `iaas-3tier-stage`, `iaas-3tier-prod`처럼 state를 분리한다.
...
아래 명령은 `infra/envs/iaas-3tier-dev` stack을 추가한 뒤 사용한다.

```bash
cp ./infra/envs/iaas-3tier-dev/terraform.tfvars.example ./infra/envs/iaas-3tier-dev/terraform.tfvars

terraform -chdir=infra/envs/iaas-3tier-dev init -backend=false
terraform -chdir=infra/envs/iaas-3tier-dev fmt -recursive
terraform -chdir=infra/envs/iaas-3tier-dev validate
terraform -chdir=infra/envs/iaas-3tier-dev plan -out=tfplan
terraform -chdir=infra/envs/iaas-3tier-dev show -json tfplan > ../../../harness/out/iaas-3tier-dev-plan.json
```

적용은 plan 검토와 승인 후 실행한다.

```bash
terraform -chdir=infra/envs/iaas-3tier-dev apply tfplan
```
