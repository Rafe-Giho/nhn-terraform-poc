...
네트워크 기준은 [NHN Cloud 네트워크 표준 설계](./architecture/network.md), 보안 기준은 [NHN Cloud 보안 표준 설계](./architecture/security.md)에 분리했다. 기본형은 Public VPC/Private VPC 2개가 아니라 `VPC 1개 + 다중 routing table + 역할별 subnet`이다.
...
이 구조는 기존 업무시스템을 Web/WAS/DB VM 계층으로 이전하는 전환 사업에 맞춘다. 하나의 VPC 안에서 `public`, `private`, `management` routing table을 분리하고 DMZ, Web, Application, Data, Management/Operations subnet을 역할별로 배치한다.

주요 Terraform 범위:

- VPC, public/private/management routing table, 역할별 subnet, security group
...
- VPN, 전용회선, Transit Hub, NAT Gateway, Service Gateway
- Network ACL, Flow Log, Load Balancer IP Access Control
...
이 구조는 NKS 기반 서비스 플랫폼, GitOps, CI/CD, DevOps 연동 서버, Object Storage를 중심으로 애플리케이션을 컨테이너화하는 전환 사업에 맞춘다. 하나의 VPC 안에서 ingress, NKS worker, DevOps, management subnet을 routing table 기준으로 분리하고 NHN Cloud foundation stack과 Kubernetes platform stack을 나눈다.

주요 Terraform 범위:

- VPC, public/private/management routing table, ingress/NKS/devops/management subnet, security group
- Object Storage, NKS cluster, NKS nodegroup
- GitLab/Gitea/Jenkins 같은 DevOps 연동 서버용 compute instance와 data volume
...
- NAT Gateway, Service Gateway, Network ACL, Flow Log
...
- Security Group은 생성 시 자동 추가되는 전체 outbound rule을 삭제하고, 표준 계층별 egress만 명시한다.
- 클라우드 네이티브의 GitLab/Gitea/Jenkins는 NHN Cloud 관리형 리소스가 아니므로 VM/볼륨/보안 그룹까지만 Terraform foundation stack에서 표준화하고, 설치와 credential은 별도 절차로 관리한다.
- NKS 내부는 `Argo CD`, `cert-manager`, CI runner/Jenkins agent, Gateway/Ingress, StorageClass, observability add-on을 별도 Kubernetes platform stack으로 관리한다.
...
| Internet Gateway ID | 변수 주입 | public routing table attach 대상. 기존 gateway 정책과 충돌 방지 |
...
| Network ACL/Flow Log | 콘솔/API 우선 | provider v1.0.8 표준 A 범위 밖이며 보안 검증 자료로 별도 관리 |
| NAT Gateway/Service Gateway | 콘솔 생성 후 설계 반영 | private outbound와 NHN 서비스 private 접근 기준 |
...
| NAT Gateway, Service Gateway, Network ACL, Flow Log, Transit Hub, Direct Connect, Peering/Colocation Gateway | 공식 Terraform 문서화 또는 provider 표준 A 범위 부족 | 콘솔 생성 후 ID/검증 결과 기록 또는 API 별도 검증 |
...
| [extract-provider-inventory.sh](../../harness/scripts/extract-provider-inventory.sh) | provider 코드의 `ResourcesMap`, `DataSourcesMap` 추출 |
| [static-check.sh](../../harness/scripts/static-check.sh) | `fmt`, `init -backend=false`, `validate`, `tflint`, `checkov/tfsec` 실행 |
| [plan-json.sh](../../harness/scripts/plan-json.sh) | `terraform plan`과 plan JSON 생성 |
| [policy-check.sh](../../harness/scripts/policy-check.sh) | 공개 SSH/RDP, 공개 전체 egress, 고위험 delete/replace, gateway attachment 점검 |
...
2. 정적 검증
3. plan JSON 생성
4. 정책 점검과 수동 리뷰로 공개 인바운드, 공개 전체 egress, destroy, 과도한 Floating IP, 운영 `-target` 사용 여부 확인
...
4. `harness/scripts/static-check.sh` 실행
